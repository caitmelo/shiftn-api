...
(async()=>{
 ...
 for(const name of ['test_engine','test_consensus'])py.FS.writeFile('/home/pyodide/'+name+'.py',fs.readFileSync('python_tests/'+name+'.py'));
 py.runPython("import unittest, test_engine, test_consensus\nsuite=unittest.TestSuite([unittest.defaultTestLoader.loadTestsFromModule(m) for m in (test_engine,test_consensus)])\nassert unittest.TextTestRunner().run(suite).wasSuccessful()");
 if(photo){
